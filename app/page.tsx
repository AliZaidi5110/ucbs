...
import HeroZellerStyle from "./components/HeroZellerStyle";
import ShopTerminalSection from "./components/ShopTerminalSection";
...
export default function HomePage() {
  return (
    <>
      {/* 1. VISUAL HERO WITH LAPTOP & MOBILE IMAGES + FLOATING CARD (MATCHING SCREENSHOT) */}
      <HeroZellerStyle />

      {/* 2. DEDICATED TERMINAL SHOP & EXPRESS ORDER NOW SECTION */}
      <ShopTerminalSection />

      {/* 3. PAYMENT METHODS & REGULATORY TRUST BADGES */}
      <TrustBadges />

      {/* 4. SYNERGY: HOW CARD MACHINE BUSINESS FUNDING WORKS */}
      <SynergySection />

      {/* 5. FAST WAYS TO REACH US (CALL 029 2271 6852, WHATSAPP, 15-MIN CALLBACK) */}
      <QuickContactHub />

      {/* 6. INTERACTIVE FUNDING & FEE SAVINGS CALCULATOR */}
      <FundingCalculator />
      ...
      {/* 11. FINAL CONVERSION ACTION SECTION */}
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 py-20 md:py-28 text-white border-t border-slate-800">
        ...
        <div className="relative mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 text-center">
          <div className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 px-3.5 py-1 text-xs font-bold text-emerald-400 mb-6">
            <span>⚡ Next-Day Terminal Dispatch</span>
          </div>
          ...
          <div className="mt-10 flex flex-wrap justify-center items-center gap-4">
            <Link
              href="/#shop-terminals"
              className="inline-flex min-h-[52px] items-center rounded-full bg-emerald-500 px-8 py-3.5 text-base font-bold text-slate-950 shadow-xl shadow-emerald-500/25 transition-all hover:bg-emerald-400 hover:shadow-emerald-400/40 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-400"
            >
              Order Terminal Online Now →
            </Link>
            
            <a
              href="tel:+442922716852"
              className="inline-flex min-h-[52px] items-center gap-2 rounded-full border border-slate-700 bg-slate-900/80 px-7 py-3.5 text-base font-semibold text-white transition-all hover:bg-slate-800 hover:border-slate-600 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white"
            >
          ...
          <div className="mt-12 flex flex-wrap justify-center items-center gap-8 text-xs text-slate-400 font-medium">
